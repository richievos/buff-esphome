...
static const uint16_t BUFF_DOSER_COMMAND_NONE = 0;
static const uint16_t BUFF_DOSER_COMMAND_TYPE_READ = 1;

static const uint16_t BUFF_DOSER_COMMAND_FIND = 2;
static const uint16_t BUFF_DOSER_COMMAND_DOSE_CONTINUOUSLY = 4;
static const uint16_t BUFF_DOSER_COMMAND_DOSE_VOLUME = 8;
static const uint16_t BUFF_DOSER_COMMAND_DOSE_VOLUME_OVER_TIME = 16;
static const uint16_t BUFF_DOSER_COMMAND_DOSE_WITH_CONSTANT_FLOW_RATE = 32;
static const uint16_t BUFF_DOSER_COMMAND_SET_CALIBRATION_VOLUME = 64;
static const uint16_t BUFF_DOSER_COMMAND_CLEAR_TOTAL_VOLUME_DOSED = 128;
static const uint16_t BUFF_DOSER_COMMAND_CLEAR_CALIBRATION = 256;
static const uint16_t BUFF_DOSER_COMMAND_PAUSE_DOSING = 512;
static const uint16_t BUFF_DOSER_COMMAND_STOP_DOSING = 1024;
static const uint16_t BUFF_DOSER_COMMAND_CHANGE_I2C_ADDRESS = 2048;
static const uint16_t BUFF_DOSER_COMMAND_EXEC_ARBITRARY_COMMAND_ADDRESS = 4096;

static const uint16_t BUFF_DOSER_COMMAND_READ_DOSING = 3;
static const uint16_t BUFF_DOSER_COMMAND_READ_SINGLE_REPORT = 5;
static const uint16_t BUFF_DOSER_COMMAND_READ_MAX_FLOW_RATE = 9;
static const uint16_t BUFF_DOSER_COMMAND_READ_PAUSE_STATUS = 17;
static const uint16_t BUFF_DOSER_COMMAND_READ_TOTAL_VOLUME_DOSED = 33;
static const uint16_t BUFF_DOSER_COMMAND_READ_ABSOLUTE_TOTAL_VOLUME_DOSED = 65;
static const uint16_t BUFF_DOSER_COMMAND_READ_CALIBRATION_STATUS = 129;
static const uint16_t BUFF_DOSER_COMMAND_READ_PUMP_VOLTAGE = 257;
...
void BuffDoser::update() {
  ...
  //   this->queue_command_(BUFF_DOSER_COMMAND_READ_CALIBRATION_STATUS, 0, 0, (bool) this->calibration_status_);
  //   this->queue_command_(BUFF_DOSER_COMMAND_READ_MAX_FLOW_RATE, 0, 0, (bool) this->max_flow_rate_);
  //   this->queue_command_(BUFF_DOSER_COMMAND_READ_SINGLE_REPORT, 0, 0, (bool) this->current_volume_dosed_);
  //   this->queue_command_(BUFF_DOSER_COMMAND_READ_TOTAL_VOLUME_DOSED, 0, 0, (bool) this->total_volume_dosed_);
  //   this->queue_command_(BUFF_DOSER_COMMAND_READ_ABSOLUTE_TOTAL_VOLUME_DOSED, 0, 0,
  //                        (bool) this->absolute_total_volume_dosed_);
  //   this->queue_command_(BUFF_DOSER_COMMAND_READ_PAUSE_STATUS, 0, 0, true);
  //   this->is_first_read_ = false;
  // }

  // if (!this->is_waiting_ && this->peek_next_command_() == BUFF_DOSER_COMMAND_NONE) {
  //   this->queue_command_(BUFF_DOSER_COMMAND_READ_DOSING, 0, 0, true);
  ...
  //   this->queue_command_(BUFF_DOSER_COMMAND_READ_PUMP_VOLTAGE, 0, 0, (bool) this->pump_voltage_);
  // } else {
...
void BuffDoser::clear_current_command_() {
  this->current_command_ = BUFF_DOSER_COMMAND_NONE;
  this->is_waiting_ = false;
}

void BuffDoser::read_command_result_() {
  ...
  switch (this->current_command_) {
    // Read Commands
    // case BUFF_DOSER_COMMAND_READ_DOSING:  // Page 54
    ...
    // case BUFF_DOSER_COMMAND_READ_SINGLE_REPORT:  // Single Report (page 53)
    ...
    // case BUFF_DOSER_COMMAND_READ_MAX_FLOW_RATE:  // Constant Flow Rate (page 57)
    ...
    // case BUFF_DOSER_COMMAND_READ_PAUSE_STATUS:  // Pause (page 61)
    ...
    // case BUFF_DOSER_COMMAND_READ_TOTAL_VOLUME_DOSED:  // Total Volume Dispensed (page 64)
    ...
    // case BUFF_DOSER_COMMAND_READ_ABSOLUTE_TOTAL_VOLUME_DOSED:  // Total Volume Dispensed (page 64)
    ...
    // case BUFF_DOSER_COMMAND_READ_CALIBRATION_STATUS:  // Calibration (page 65)
    ...
    // case BUFF_DOSER_COMMAND_READ_PUMP_VOLTAGE:  // Pump Voltage (page 67)
    ...
    // case BUFF_DOSER_COMMAND_DOSE_VOLUME:  // Volume Dispensing (page 55)
    //   if (this->dosing_mode_ && this->dosing_mode_->state != DOSING_MODE_VOLUME)
    //     this->dosing_mode_->publish_state(DOSING_MODE_VOLUME);
    //   break;

    // case BUFF_DOSER_COMMAND_DOSE_VOLUME_OVER_TIME:  // Dose over time (page 56)
    ...
    // case BUFF_DOSER_COMMAND_DOSE_WITH_CONSTANT_FLOW_RATE:  // Constant Flow Rate (page 57)
    ...
    // case BUFF_DOSER_COMMAND_DOSE_CONTINUOUSLY:  // Continuous Dispensing (page 54)
    ...
    // case BUFF_DOSER_COMMAND_STOP_DOSING:  // Stop (page 62)
    ...
    // case BUFF_DOSER_COMMAND_EXEC_ARBITRARY_COMMAND_ADDRESS:
    //   ESP_LOGI(TAG, "Arbitrary Command Response: %s", (char *) response_buffer);
    //   break;

    // case BUFF_DOSER_COMMAND_CLEAR_CALIBRATION:         // Clear Calibration (page 65)
    // case BUFF_DOSER_COMMAND_PAUSE_DOSING:              // Pause (page 61)
    // case BUFF_DOSER_COMMAND_SET_CALIBRATION_VOLUME:    // Set Calibration Volume (page 65)
    // case BUFF_DOSER_COMMAND_CLEAR_TOTAL_VOLUME_DOSED:  // Clear Total Volume Dosed (page 64)
    // case BUFF_DOSER_COMMAND_FIND:                      // Find (page 52)
    //   // Nothing to do here
    //   break;

    // case BUFF_DOSER_COMMAND_TYPE_READ:
    // case BUFF_DOSER_COMMAND_NONE:
    default:
...
void BuffDoser::send_next_command_() {
  ...
  switch (this->next_command_) {
    ...
    // case BUFF_DOSER_COMMAND_READ_MAX_FLOW_RATE:
    //   command_buffer_length = sprintf((char *) command_buffer, "DC,?");
    //   break;

    // case BUFF_DOSER_COMMAND_READ_PAUSE_STATUS:
    //   command_buffer_length = sprintf((char *) command_buffer, "P,?");
    //   break;

    // case BUFF_DOSER_COMMAND_READ_TOTAL_VOLUME_DOSED:
    //   command_buffer_length = sprintf((char *) command_buffer, "TV,?");
    //   break;

    // case BUFF_DOSER_COMMAND_READ_ABSOLUTE_TOTAL_VOLUME_DOSED:
    //   command_buffer_length = sprintf((char *) command_buffer, "ATV,?");
    //   break;

    // case BUFF_DOSER_COMMAND_READ_CALIBRATION_STATUS:
    //   command_buffer_length = sprintf((char *) command_buffer, "Cal,?");
    //   break;

    // case BUFF_DOSER_COMMAND_READ_PUMP_VOLTAGE:
    ...
    // case BUFF_DOSER_COMMAND_CHANGE_I2C_ADDRESS:  // Change I2C Address (page 73)
    //   command_buffer_length = sprintf((char *) command_buffer, "I2C,%i", this->next_command_duration_);
    //   break;

    // case BUFF_DOSER_COMMAND_EXEC_ARBITRARY_COMMAND_ADDRESS:  // Run an arbitrary command
  ...
  this->next_command_ = BUFF_DOSER_COMMAND_NONE;
  this->is_waiting_ = true;
  this->start_time_ = millis();
  this->wait_time_ = wait_time_for_command;
}

void BuffDoser::pop_next_command_() {
...
uint16_t BuffDoser::peek_next_command_() {
  if (this->next_command_queue_length_ <= 0) {
    return BUFF_DOSER_COMMAND_NONE;
  }

  return this->next_command_queue_[this->next_command_queue_head_];
}

void BuffDoser::queue_command_(uint16_t command, double volume, int duration, bool should_schedule) {
  if (!should_schedule) {
    return;
  }
...
void BuffDoser::find() { this->queue_command_(BUFF_DOSER_COMMAND_FIND, 0, 0, true); }

void BuffDoser::dose_continuously() {
  this->queue_command_(BUFF_DOSER_COMMAND_DOSE_CONTINUOUSLY, 0, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_DOSING, 0, 0, true);
  // this->queue_command_(BUFF_DOSER_COMMAND_READ_SINGLE_REPORT, 0, 0, (bool) this->current_volume_dosed_);
}

void BuffDoser::dose_volume(double volume) {
  this->queue_command_(BUFF_DOSER_COMMAND_DOSE_VOLUME, volume, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_DOSING, 0, 0, true);
  // this->queue_command_(BUFF_DOSER_COMMAND_READ_SINGLE_REPORT, 0, 0, (bool) this->current_volume_dosed_);
}

void BuffDoser::dose_volume_over_time(double volume, int duration) {
  this->queue_command_(BUFF_DOSER_COMMAND_DOSE_VOLUME_OVER_TIME, volume, duration, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_DOSING, 0, 0, true);
  // this->queue_command_(BUFF_DOSER_COMMAND_READ_SINGLE_REPORT, 0, 0, (bool) this->current_volume_dosed_);
}

void BuffDoser::dose_with_constant_flow_rate(double volume, int duration) {
  this->queue_command_(BUFF_DOSER_COMMAND_DOSE_WITH_CONSTANT_FLOW_RATE, volume, duration, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_DOSING, 0, 0, true);
  // this->queue_command_(BUFF_DOSER_COMMAND_READ_SINGLE_REPORT, 0, 0, (bool) this->current_volume_dosed_);
}

void BuffDoser::set_calibration_volume(double volume) {
  this->queue_command_(BUFF_DOSER_COMMAND_SET_CALIBRATION_VOLUME, volume, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_CALIBRATION_STATUS, 0, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_MAX_FLOW_RATE, 0, 0, true);
}

void BuffDoser::clear_total_volume_dosed() {
  this->queue_command_(BUFF_DOSER_COMMAND_CLEAR_TOTAL_VOLUME_DOSED, 0, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_SINGLE_REPORT, 0, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_TOTAL_VOLUME_DOSED, 0, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_ABSOLUTE_TOTAL_VOLUME_DOSED, 0, 0, true);
}

void BuffDoser::clear_calibration() {
  this->queue_command_(BUFF_DOSER_COMMAND_CLEAR_CALIBRATION, 0, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_CALIBRATION_STATUS, 0, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_MAX_FLOW_RATE, 0, 0, true);
}

void BuffDoser::pause_dosing() {
  this->queue_command_(BUFF_DOSER_COMMAND_PAUSE_DOSING, 0, 0, true);
  this->queue_command_(BUFF_DOSER_COMMAND_READ_PAUSE_STATUS, 0, 0, true);
}

void BuffDoser::stop_dosing() { this->queue_command_(BUFF_DOSER_COMMAND_STOP_DOSING, 0, 0, true); }

void BuffDoser::change_i2c_address(int address) {
  this->queue_command_(BUFF_DOSER_COMMAND_CHANGE_I2C_ADDRESS, 0, address, true);
}

void BuffDoser::exec_arbitrary_command(const std::basic_string<char> &command) {
  this->arbitrary_command_ = command.c_str();
  this->queue_command_(BUFF_DOSER_COMMAND_EXEC_ARBITRARY_COMMAND_ADDRESS, 0, 0, true);
}
